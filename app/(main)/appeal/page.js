...
async function getAppeals() {
  ...
  try {
    ...
    return Array.isArray(data) ? data : data.appeals || MOCK_APPEALS;
  } catch {
    return MOCK_APPEALS;
  }
}
    ...
    <div className="mx-auto w-full max-w-[1440px] space-y-6">
      <section className="surface-panel rounded-lg px-4 py-5 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          ...
          <Link href="/appeal/new" className="inline-flex min-h-10 items-center justify-center rounded-md bg-[var(--navy-600)] px-4 text-sm font-bold text-white transition hover:bg-[var(--navy-800)]">
            ＋ 新增申訴案件
          </Link>
        </div>
      </section>
      ...
      <section className="surface-panel overflow-hidden rounded-lg">
        {!appeals.length ? (
          <div className="p-8 text-center text-sm text-slate-500">目前沒有申訴紀錄，點右上角「新增申訴案件」建立。</div>
        ) : (
          <div className="divide-y divide-slate-100">
            {appeals.map((a) => {
              const meta = statusMeta(a.status);
              return (
                <Link key={a.id} href={`/appeal/${a.id}`} className="grid gap-3 p-5 transition hover:bg-[var(--surface-muted)] lg:grid-cols-[1fr_auto] lg:items-center">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-sm text-slate-600">訂單 {a.order_id}・{REASON_LABELS[a.reason] || a.reason}</p>
                    <p className="mt-1 line-clamp-1 text-sm text-slate-500">{a.message}</p>
                    <p className="mt-2 text-xs font-semibold text-slate-400">{formatDate(a.created_at)}</p>
                  </div>
