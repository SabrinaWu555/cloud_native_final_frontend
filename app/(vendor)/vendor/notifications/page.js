      ...
      <section className="surface-panel rounded-lg px-4 py-5 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-600">掌握訂單建立與取消的最新動態。</p>
          </div>
          <div className="flex items-center gap-3">
            <Link
              href="/vendor"
              className="text-sm font-semibold text-slate-500 transition hover:text-[var(--navy-600)]"
            >
              ← 返回工作台
            </Link>
          ...
          <div className="divide-y divide-slate-100">
            {items.map((item) => {
              ...
              return (
                <Link
                  key={item.id}
                  href={`/vendor/notifications/${item.id}`}
                ...
                >
                  ...
                  <span className="hidden text-sm font-bold text-[var(--navy-600)] lg:block">
                    查看內容 →
                  </span>
                </Link>
              );
